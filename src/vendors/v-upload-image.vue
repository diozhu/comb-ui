<template>
    ...
    <div class="v-upload-image multi">
        <div v-for="(item, index) in currentValue" :key="index">
            <div
                :class="['frm', { loading: item.loading, fadeIn: !item.loading, one: max == 1, gray: item.base64 && !item.url }]"
                :style="{
            ...
            >
                ...
                <img
                    v-if="item.url || item.thumb"
                    :src="item.thumb || item.url"
                    @click="handleTapPreview(item, index)"
                />
        ...
        <button
            v-if="isApp"
            :disabled="disabled"
            v-show="currentValue.length < max || max == 0"
            @click="appChooseImage"
            class="wx_upload_button wechat frm"
            :class="[{ one: max == 1 }]"
        >
...
export default {
    ...
    methods: {
        ...
        appChooseImage() {
            ...
            //     [].forEach.call(this.headers, v => {
            //         xhttp.setRequestHeader(v.key, v.value);
            //     });
            //     xhttp.withCredentials = true;
            //     xhttp.responseType = 'json';
            //     xhttp.onload = function () {
            //         if (xhttp.status === 200) {
            //             window.alert(JSON.stringify(xhttp.response));
            ...
            //     xhttp.send(data);
            // };
            let uploadImageSuccessFunc = (params, newPhoto) => {
                    return this.upFunc(params)
                        .then(res => {
                            if (!res) return;
                            let num = 0;
                            _self.currentValue.forEach((v, i) => {
                                if (v.returnReq === params.returnReq) {
                                    let obj = {
                                        url: utils.format(res.filePath, {width: 160, height: 160}),
                                        orgurl: res.filePath,
                                        loading: false,
                                        imaId: res.id,
                            ...
                            syncUploadImage(newPhoto);
                            return Promise.resolve(res);
                        })
                        .catch(e => {
                            _self.currentValue.forEach(v => {
                                if (v.returnReq === params.returnReq) {
                                    // v.base64 = '';
                                    v.loading = false;
                                }
                ...
                syncUploadImage = (newPhoto) => {
                    if (!newPhoto || !newPhoto.length) return;
                    let param = newPhoto.pop();
                    uploadImageSuccessFunc(param, newPhoto);
                };
            let count = this.max - this.value.length;
            if (this.max === 0 || this.max === '0') count = 9;
            Bridge.chooseImage({ count: count }).then(res => {
                this.uploadedStatus = false;
                if (parseInt(this.max) === 1) this.currentValue = [];
                let newPhoto = [];
                res.forEach((v, i) => {
                    newPhoto.push({ base64: 'data:image/jpeg;base64,' + v.base64, returnReq: i + 1 });
                    this.currentValue.push({
                        url: '',
                        base64: 'data:image/jpeg;base64,' + v.base64,
                        loading: true,
                        returnReq: i + 1,
                        mediaId: ''
                    });
                ...
                if (typeof this.upFunc !== 'function') return;
                syncUploadImage(newPhoto)
                // return this.upFunc(param).then(rtn => {
        ...
        h5ChooseImage(e) {
            ...
            try {
                if (e.target) {
                    ...
                    //     console.error('aaaaaa', file);
                    // };
                    // console.error('aaaaaa', reader.readAsDataURL(fileInput.files[0]));
                    if (typeof this.upFunc !== 'function') return;
                    return this.upFunc({'file': fileInput.files[0]})
                        .then(res => {
                            if (!res) return;
                            //                    请求返回后不显示默认图片；
                    ...
                    //                     if (v.loading) {
                    //                         _self.$set(_self.currentValue, i, {
                    //                             url: utils.format(xhr.response.data.filePath, {
                    //                                 width: 160,
                    //                                 height: 160
                    //                             }),
