...
let validate = function () {
        ...
        if (this.field) {
            /*
            * 验证的时候，删除内容两侧的空白
            *   Author By 刘俊俊 2018.7.16
            * */
            // let value = this.vm.value.trim();
            ...
            if (this.expression.phone) _validatePhone.call(this, this.vm.value);
            ...
            if (this.expression.image) _validateImage.call(this, this.vm.value);
            if (this.expression.relation) _validateRelation.call(this, this.vm.value);
        }
    },
    ...
    _validateRequired = function (newVal, oldVal) { // 监听输入与否
        /*
        * newVal可能是数字，那么下面的判断 newVal.length就会出问题，所以将数字转换为字符串验证
        *   Modify By 刘俊俊 2018.6.11
        * */
    ...
    _validatePhone = function (newVal, oldVal) { // 监听输入长度
        if (newVal && this.expression.phone) {
            if (utils.validatePhone(newVal)) {
                delete Vue.$validation[this.field]['phone'];
                delete Vue.prototype.$validation[this.field]['phone'];
            } else {
                let msg = _getMsg.call(this, 'phone', '手机格式不对哦~');
                Vue.$validation[this.field]['phone'] = Vue.prototype.$validation[this.field]['phone'] = msg;
            }
        }
        if (!newVal && !this.expression.required) {
            delete Vue.$validation[this.field]['phone'];
    ...
    // 验证两张证件照
    _validateImage = function (newVal, oldVal) { // 监听输入长度
        if (newVal && this.expression.image) {
            if (newVal.length === 2) {
                delete Vue.$validation[this.field]['image'];
                delete Vue.prototype.$validation[this.field]['image'];
            } else {
                let msg = _getMsg.call(this, 'image', '请上传两张证件照~');
                Vue.$validation[this.field]['image'] = Vue.prototype.$validation[this.field]['image'] = msg;
            }
        }
        if (!newVal && !this.expression.required) {
            delete Vue.$validation[this.field]['image'];
    ...
    // 检验与甲方关系
    _validateRelation = function (newVal, oldVal) { // 监听输入长度
        if (newVal && this.expression.relation) {
            if (newVal.key !== undefined) {
                delete Vue.$validation[this.field]['relation'];
                delete Vue.prototype.$validation[this.field]['relation'];
            } else {
                let msg = _getMsg.call(this, 'relation', '请选择与甲方关系');
                Vue.$validation[this.field]['relation'] = Vue.prototype.$validation[this.field]['relation'] = msg;
            }
        }
        if (!newVal && !this.expression.required) {
            delete Vue.$validation[this.field]['relation'];
